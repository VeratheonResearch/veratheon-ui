...
<div class="card border border-base-200 bg-base-100 shadow-lg">
	<div class="card-body p-6">
		<h2 class="mb-4 card-title text-xl font-bold text-primary">
			Trade Hypothesis
			{#if symbol}
		...
		{#if companyName}
			<p class="mb-4 text-sm text-base-content/70">{companyName}</p>
		{/if}
		...
		<form onsubmit={handleSubmit} class="space-y-4">
			...
			<div class="form-control">
				...
				<div
					class="join w-full"
					id="instrument-type-group"
					role="radiogroup"
					aria-labelledby="instrument-type-label"
				>
					<button
						type="button"
						class="btn join-item flex-1 {instrumentType === 'stock'
							? 'btn-primary'
							: 'btn-outline'}"
						onclick={() => (instrumentType = 'stock')}
					...
					<button
						type="button"
						class="btn join-item flex-1 {instrumentType === 'option'
							? 'btn-primary'
							: 'btn-outline'}"
						onclick={() => (instrumentType = 'option')}
			...
			{#if instrumentType === 'stock'}
				<div class="form-control">
					...
					>
						<button
							type="button"
							class="btn join-item flex-1 {direction === 'long' ? 'btn-success' : 'btn-outline'}"
						...
						<button
							type="button"
							class="btn join-item flex-1 {direction === 'short' ? 'btn-error' : 'btn-outline'}"
			...
			{#if instrumentType === 'option'}
				...
				<div class="form-control">
					...
					<div
						class="join w-full"
						id="option-type-group"
						role="radiogroup"
						aria-labelledby="option-type-label"
					>
						<button
							type="button"
							class="btn join-item flex-1 {optionType === 'call' ? 'btn-success' : 'btn-outline'}"
							onclick={() => (optionType = 'call')}
						...
						<button
							type="button"
							class="btn join-item flex-1 {optionType === 'put' ? 'btn-error' : 'btn-outline'}"
							onclick={() => (optionType = 'put')}
				...
				<div class="form-control">
					...
					<input
						...
						class="input-bordered input w-full {errors.strikePrice && formSubmitted
							? 'input-error'
							: ''}"
				...
				<div class="form-control">
					...
					<input
						...
						type="date"
						min={today}
						class="input-bordered input w-full {errors.expirationDate && formSubmitted
							? 'input-error'
							: ''}"
			...
			<div class="form-control">
				<label class="label cursor-pointer">
					<div class="flex items-center gap-2">
						...
						>
							<HelpCircle class="h-4 w-4 cursor-help text-info" />
						</div>
					</div>
					<input type="checkbox" class="checkbox checkbox-primary" bind:checked={isHedge} />
				</label>
			</div>
